...
from metabutler import dispatcher, LOGGER, OWNER_ID, SUDO_USERS, TEMPORARY_DATA
...
from metabutler.modules.helper_funcs.string_handling import button_markdown_parser, make_time

# SQL
import metabutler.modules.sql.antiflood_sql as antifloodsql
import metabutler.modules.sql.blacklist_sql as blacklistsql
import metabutler.modules.sql.blsticker_sql as blackliststksql
...
from metabutler.modules.locks import LOCK_TYPES
from metabutler.modules.sql import notes_sql as notesql
from metabutler.modules.sql import reporting_sql as reportsql
...
def import_data(update, context):
	...
	if msg.reply_to_message and msg.reply_to_message.document:
		...
		try:
			# If backup is from metabutler
			if data.get('bot_base') == "Metabutler":
				imp_antiflood = False
				imp_blacklist = False
			...
			send_message(update.effective_message, "An exception occured while restoring your data from Metabutler backup!", parse_mode="markdown")
			LOGGER.exception("An error when importing from Metabutler base!")
			return
...
def export_data(update, context):
	...
	try:
		...
		pass
	os.remove("{}-Metabutler.backup".format(chat_id)) # Cleaning file
...
__help__ = """
*Admin only:*
 - /import: reply to a group butler/marie/rose/metabutler backup file to import as much as possible, making the transfer super simple!
Note that files/photos from other bots can't be imported due to telegram restrictions. Except for Metabutler backup it self.
